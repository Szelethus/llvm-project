...
void ExprEngine::processBranch(
  ...
  for (ExplodedNode *PredN : CheckersOutSet) {
    ...
    if (this->AMgr.getAnalyzerOptions().IsPathSensitive) {
      ...
      if (StTrue) {
        ...
        // false positives if e.g. the loop iterates over a two-element
        // structure with an opaque condition.
        //
        ...
        bool CompletedTwoIterations =
            IterationsCompletedInLoop.value_or(0) >= 2;
