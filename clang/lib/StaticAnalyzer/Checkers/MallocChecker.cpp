...
struct AllocationFamily {
    ...
    bool operator==(const AllocationFamily &Other) const {
        return std::tie(Kind, CustomName) ==
               std::tie(Other.Kind, Other.CustomName);
    }
...
class RefState {
  ...
  public:
    ...
    static RefState getAllocatedOfSizeZero(const RefState *RS) {
        return RefState(
            AllocatedOfSizeZero, RS->getStmt(), RS->getAllocationFamily());
    }
...
MallocUpdateRefState(CheckerContext &C,
                     const Expr *E,
...
class MallocChecker : public Checker<check::DeadSymbols,
  ...
  public:
    ...
    void printState(raw_ostream &Out,
                    ProgramStateRef State,
                    const char *NL,
...
#define CHECK_FN(NAME)                                                         \
    ...
    void checkRealloc(ProgramStateRef State,
                      const CallEvent &Call,
                      CheckerContext &C,
    ...
    CallDescriptionMap<CheckFn> AllocatingMemFnMap{
        ...
        {{CDM::CLibrary, {"if_nameindex"}, 1},
         &MallocChecker::checkIfNameIndex},
        ...
        {{CDM::CLibrary, {"g_try_malloc0_n"}, 2},
         &MallocChecker::checkGMallocN0},
    };
    ...
    /// Optional is empty, afterwards it contains another Optional that
    /// indicates if the macro value could be determined, and if yes the value
    /// itself.
    ...
    processNewAllocation(const CXXAllocatorCall &Call,
                         CheckerContext &C,
                         AllocationFamily Family) const;
    ...
    ProcessZeroAllocCheck(CheckerContext &C,
                          const CallEvent &Call,
    ...
    [[nodiscard]] ProgramStateRef
    MallocMemReturnsAttr(CheckerContext &C,
                         const CallEvent &Call,
    ...
    performKernelMalloc(const CallEvent &Call,
                        CheckerContext &C,
                        const ProgramStateRef &State) const;

    /// Model functions with the ownership_takes and ownership_holds attributes.
    ///
    /// User-defined function may have the ownership_takes and/or
    /// ownership_holds attributes, which annotates that the function frees the
    /// memory passed as a parameter.
    ///
    ...
    /// ownership_holds
    ///   attribute.
    /// \param [out] IsKnownToBeAllocated Whether the memory to be freed is
    /// known
    ...
    ///        // ...
    ///      }
    /// \param [in] ReturnsNullOnFailure Whether the memory deallocation
    /// function
    ///   we're modeling returns with Null on failure.
    /// \returns The ProgramState right after deallocation.
    [[nodiscard]] ProgramStateRef
    FreeMemAux(CheckerContext &C,
               const CallEvent &Call,
               ProgramStateRef State,
               unsigned Num,
               bool Hold,
    ...
    [[nodiscard]] ProgramStateRef ReallocMemAux(CheckerContext &C,
                                                const CallEvent &Call,
    ...
    bool
    checkUseAfterFree(SymbolRef Sym, CheckerContext &C, const Stmt *S) const;

    /// If in \p S \p Sym is used, check whether \p Sym was allocated as a zero
    /// sized memory region.
    void checkUseZeroAllocated(SymbolRef Sym,
                               CheckerContext &C,
                               const Stmt *S) const;

    /// If in \p S \p Sym is being freed, check whether \p Sym was already
    /// freed.
    ...
    bool mayFreeAnyEscapedMemoryOrIsModeledExplicitly(
        const CallEvent *Call,
        ProgramStateRef State,
    ...
    std::optional<CheckKind> getCheckIfTracked(CheckerContext &C,
                                               SymbolRef Sym,
                                               bool IsALeakCheck = false) const;
    ///@}
    ...
    void HandleNonHeapDealloc(CheckerContext &C,
                              SVal ArgVal,
                              SourceRange Range,
    ...
    void
    HandleFreeAlloca(CheckerContext &C, SVal ArgVal, SourceRange Range) const;

    void HandleMismatchedDealloc(CheckerContext &C,
                                 SourceRange Range,
                                 const Expr *DeallocExpr,
                                 const RefState *RS,
                                 SymbolRef Sym,
    ...
    void HandleOffsetFree(CheckerContext &C,
                          SVal ArgVal,
                          SourceRange Range,
                          const Expr *DeallocExpr,
                          AllocationFamily Family,
    ...
    void HandleUseAfterFree(CheckerContext &C,
                            SourceRange Range,
                            SymbolRef Sym) const;

    void HandleDoubleFree(CheckerContext &C,
                          SourceRange Range,
                          bool Released,
                          SymbolRef Sym,
    ...
    void HandleUseZeroAlloc(CheckerContext &C,
                            SourceRange Range,
                            SymbolRef Sym) const;

    void HandleFunctionPtrFree(CheckerContext &C,
                               SVal ArgVal,
                               SourceRange Range,
                               const Expr *FreeExpr,
                               AllocationFamily Family) const;
    ...
    /// exploded node N.
    static LeakInfo
    getAllocationSite(const ExplodedNode *N, SymbolRef Sym, CheckerContext &C);
    ...
    bool isArgZERO_SIZE_PTR(ProgramStateRef State,
                            CheckerContext &C,
                            SVal ArgVal) const;
};
...
class NoMemOwnershipChangeVisitor final : public NoOwnershipChangeVisitor {
  protected:
    /// Syntactically checks whether the callee is a deallocating function.
    /// Since we have no path-sensitive information on this call (we would need
    /// a CallEvent instead of a CallExpr for that), its possible that a
    ...
    bool doesFnIntendToHandleOwnership(const Decl *Callee,
        ...
        // put notes inside it, given that it doesn't match any source
        // locations?
        if (!FD || !FD->hasBody())
            return false;
...
class MallocBugVisitor final : public BugReporterVisitor {
  ...
  public:
    ...
    static inline bool isAllocated(const RefState *RSCurr,
    ...
    static inline bool isReleased(const RefState *RSCurr,
                                  const RefState *RSPrev,
                                  const Stmt *Stmt) {
        bool IsReleased = (RSCurr && RSCurr->isReleased()) &&
                          (!RSPrev || !RSPrev->isReleased());
        assert(!IsReleased ||
               (isa_and_nonnull<CallExpr, CXXDeleteExpr>(Stmt)) ||
    ...
    static inline bool isRelinquished(const RefState *RSCurr,
                                      const RefState *RSPrev,
                                      const Stmt *Stmt) {
        return (isa_and_nonnull<CallExpr, ObjCMessageExpr, ObjCPropertyRefExpr>(
                    Stmt) &&
      ...
      public:
        ...
        std::string getMessageForArg(const Expr *ArgE,
            ...
            os << "Reallocation of " << ArgIndex
               << llvm::getOrdinalSuffix(ArgIndex) << " parameter failed";
...
MallocChecker::performKernelMalloc(const CallEvent &Call,
                                   CheckerContext &C,
                                   const ProgramStateRef &State) const {
    ...
    // We treat the last argument as the flags argument, and callers fall-back
    // to normal malloc on a None return. This works for the FreeBSD kernel
    // malloc as well as Linux kmalloc.
...
SVal MallocChecker::evalMulForBufferSize(CheckerContext &C,
    ...
    SVal TotalSize = SB.evalBinOp(
        State, BO_Mul, BlocksVal, BlockBytesVal, SB.getContext().getSizeType());
    return TotalSize;
}
...
static bool isStandardRealloc(const CallEvent &Call) {
    ...
    return FD->getDeclaredReturnType().getDesugaredType(AC) == AC.VoidPtrTy &&
           FD->getParamDecl(0)->getType().getDesugaredType(AC) ==
               AC.VoidPtrTy &&
...
void MallocChecker::checkRealloc(ProgramStateRef State,
                                 const CallEvent &Call,
                                 CheckerContext &C,
                                 bool ShouldFreeOnFail) const {
    ...
    State = ReallocMemAux(
        C, Call, ShouldFreeOnFail, State, AllocationFamily(AF_Malloc));
    State = ProcessZeroAllocCheck(C, Call, 1, State);
    C.addTransition(State);
}

void MallocChecker::checkCalloc(ProgramStateRef State,
                                const CallEvent &Call,
                                CheckerContext &C) const {
...
void MallocChecker::checkFree(ProgramStateRef State,
                              const CallEvent &Call,
                              CheckerContext &C) const {
...
void MallocChecker::checkAlloca(ProgramStateRef State,
                                const CallEvent &Call,
                                CheckerContext &C) const {
    State = MallocMemAux(C,
                         Call,
...
void MallocChecker::checkStrdup(ProgramStateRef State,
                                const CallEvent &Call,
                                CheckerContext &C) const {
...
void MallocChecker::checkGMalloc0(ProgramStateRef State,
                                  const CallEvent &Call,
                                  CheckerContext &C) const {
...
void MallocChecker::checkGMemdup(ProgramStateRef State,
                                 const CallEvent &Call,
                                 CheckerContext &C) const {
    State = MallocMemAux(C,
                         Call,
...
void MallocChecker::checkGMallocN(ProgramStateRef State,
                                  const CallEvent &Call,
                                  CheckerContext &C) const {
    ...
    State = MallocMemAux(
        C, Call, TotalSize, Init, State, AllocationFamily(AF_Malloc));
    State = ProcessZeroAllocCheck(C, Call, 0, State);
    State = ProcessZeroAllocCheck(C, Call, 1, State);
    C.addTransition(State);
}

void MallocChecker::checkGMallocN0(ProgramStateRef State,
                                   const CallEvent &Call,
                                   CheckerContext &C) const {
    SValBuilder &SB = C.getSValBuilder();
...
void MallocChecker::preGetdelim(ProgramStateRef State,
                                const CallEvent &Call,
                                CheckerContext &C) const {
...
void MallocChecker::checkGetdelim(ProgramStateRef State,
                                  const CallEvent &Call,
                                  CheckerContext &C) const {
    // Discard calls to the C++ standard library function std::getline(), which
    // is completely unrelated to the POSIX getline() that we're checking.
    ...
    C.addTransition(MallocUpdateRefState(
        C, CE, State, AllocationFamily(AF_Malloc), *LinePtr));
}

void MallocChecker::checkReallocN(ProgramStateRef State,
                                  const CallEvent &Call,
                                  CheckerContext &C) const {
    State = ReallocMemAux(C,
                          Call,
                          /*ShouldFreeOnFail=*/false,
                          State,
                          AllocationFamily(AF_Malloc),
                          /*SuffixWithN=*/true);
    State = ProcessZeroAllocCheck(C, Call, 1, State);
...
ProgramStateRef
MallocChecker::ProcessZeroAllocCheck(CheckerContext &C,
                                     const CallEvent &Call,
                                     const unsigned IndexOfSizeArg,
                                     ProgramStateRef State,
    ...
    if (TrueState && !FalseState) {
        ...
        if (RS) {
            ...
            // Case of zero-size realloc. Historically 'realloc(ptr, 0)' is
            // treated as 'free(ptr)' and the returned value from 'realloc(ptr,
            // 0)' is not tracked. Add zero-reallocated Sym to the state to
            // catch references to zero-allocated memory.
...
static bool isKnownDeallocObjCMethodName(const ObjCMethodCall &Call) {
    ...
    // object takes ownership of the memory, promising to eventually deallocate
    // it with free(). Ex:  [NSData dataWithBytesNoCopy:bytes length:10];
    // (...unless a 'freeWhenDone' parameter is false, but that's checked
    // later.)
...
void MallocChecker::checkPostObjCMessage(const ObjCMethodCall &Call,
    ...
    ProgramStateRef State = FreeMemAux(C,
                                       Call.getArgExpr(0),
                                       Call,
                                       C.getState(),
                                       /*Hold=*/true,
...
ProgramStateRef
MallocChecker::MallocMemReturnsAttr(CheckerContext &C,
                                    const CallEvent &Call,
                                    const OwnershipAttr *Att,
                                    ProgramStateRef State) const {
    if (!State)
...
void MallocChecker::reportTaintBug(StringRef Msg,
                                   ProgramStateRef State,
                                   CheckerContext &C,
                                   llvm::ArrayRef<SymbolRef> TaintedSyms,
                                   AllocationFamily Family) const {
    if (ExplodedNode *N = C.generateNonFatalErrorNode(State, this)) {
        ...
        auto R =
            std::make_unique<PathSensitiveBugReport>(*BT_TaintedAlloc, Msg, N);
...
void MallocChecker::checkTaintedness(CheckerContext &C,
                                     const CallEvent &Call,
                                     const SVal SizeSVal,
                                     ProgramStateRef State,
                                     AllocationFamily Family) const {
...
ProgramStateRef MallocChecker::MallocMemAux(CheckerContext &C,
    ...
    State = setDynamicExtent(
        State, RetVal.getAsRegion(), Size.castAs<DefinedOrUnknownSVal>());
...
static void
printOwnershipTakesList(raw_ostream &os, CheckerContext &C, const Expr *E) {
...
ProgramStateRef MallocChecker::FreeMemAux(CheckerContext &C,
    ...
    // NOTE: We detected a bug, but the checker under whose name we would emit
    // the error could be disabled. Generally speaking, the MallocChecker family
    // is an integral part of the Static Analyzer, and disabling any part of it
    // should only be done under exceptional circumstances, such as frequent
    // false positives. If this is the case, we can reasonably believe that
    // there are serious faults in our understanding of the source code, and
    // even if we don't emit an warning, we should terminate further analysis
    // with a sink node.

    // Nonlocs can't be freed, of course.
    // Non-region locations (labels and fixed addresses) also shouldn't be
    // freed.
    if (!R) {
        // Exception:
        // If the macro ZERO_SIZE_PTR is defined, this could be a kernel source
        // code. In that case, the ZERO_SIZE_PTR defines a special value used
        // for a zero-sized memory block which is allowed to be freed, despite
        // not being a null pointer.
        if (Family.Kind != AF_Malloc || !isArgZERO_SIZE_PTR(State, C, ArgVal))
            HandleNonHeapDealloc(
                C, ArgVal, ArgExpr->getSourceRange(), ParentExpr, Family);
        return nullptr;
    }
    ...
    if (!isa<UnknownSpaceRegion, HeapSpaceRegion>(MS)) {
        // Regions returned by malloc() are represented by SymbolicRegion
        // objects within HeapSpaceRegion. Of course, free() can work on memory
        // allocated outside the current function, so UnknownSpaceRegion is also
        // a possibility here.
    ...
    if (RsBase) {
        ...
        if ((RsBase->isReleased() || RsBase->isRelinquished()) &&
            ...
            HandleDoubleFree(C,
                             ParentExpr->getSourceRange(),
                             RsBase->isReleased(),
                             SymBase,
                             PreviousRetStatusSymbol);
            return nullptr;

            // If the pointer is allocated or escaped, but we are now trying to
            // free it, check that the call to free is proper.
    ...
    // freeing function is the same as the symbols allocation family, and go
    // with that.
...
std::optional<MallocChecker::CheckKind> MallocChecker::getCheckIfTracked(
    CheckerContext &C, SymbolRef Sym, bool IsALeakCheck) const {
...
void MallocChecker::HandleNonHeapDealloc(CheckerContext &C,
                                         SVal ArgVal,
                                         SourceRange Range,
                                         const Expr *DeallocExpr,
                                         AllocationFamily Family) const {
    ...
    if (ExplodedNode *N = C.generateErrorNode()) {
        ...
        os << " is ";
        bool Summarized =
            MR ? SummarizeRegion(os, MR) : SummarizeValue(os, ArgVal);
        if (Summarized)
...
void MallocChecker::HandleFreeAlloca(CheckerContext &C,
                                     SVal ArgVal,
...
void MallocChecker::HandleOffsetFree(CheckerContext &C,
                                     SVal ArgVal,
                                     SourceRange Range,
                                     const Expr *DeallocExpr,
                                     AllocationFamily Family,
    ...
    auto R = std::make_unique<PathSensitiveBugReport>(
        *BT_OffsetFree[*CheckKind], os.str(), N);
    R->markInteresting(MR->getBaseRegion());
    R->addRange(Range);
    C.emitReport(std::move(R));
}

void MallocChecker::HandleUseAfterFree(CheckerContext &C,
                                       SourceRange Range,
                                       SymbolRef Sym) const {
    ...
    if (ExplodedNode *N = C.generateErrorNode()) {
        if (!BT_UseFree[*CheckKind])
            BT_UseFree[*CheckKind].reset(new BugType(CheckNames[*CheckKind],
                                                     "Use-after-free",
                                                     categories::MemoryError));
...
void MallocChecker::HandleDoubleFree(CheckerContext &C,
                                     SourceRange Range,
                                     bool Released,
                                     SymbolRef Sym,
                                     SymbolRef PrevSym) const {
...
void MallocChecker::HandleUseZeroAlloc(CheckerContext &C,
                                       SourceRange Range,
                                       SymbolRef Sym) const {
...
void MallocChecker::HandleFunctionPtrFree(CheckerContext &C,
                                          SVal ArgVal,
                                          SourceRange Range,
                                          const Expr *FreeExpr,
                                          AllocationFamily Family) const {
...
ProgramStateRef MallocChecker::ReallocMemAux(CheckerContext &C,
    ...
    DefinedOrUnknownSVal SizeZero =
        svalBuilder.evalEQ(State,
                           TotalSize.castAs<DefinedOrUnknownSVal>(),
    ...
    if (SizeIsZero)
        ...
        // passed to free() is returned. We just free the input pointer and do
        // not add any constrains on the output pointer.
        if (ProgramStateRef stateFree = FreeMemAux(C,
                                                   Call,
                                                   StateSizeIsZero,
                                                   0,
                                                   false,
    ...
    if (ProgramStateRef stateFree = FreeMemAux(
            C, Call, State, 0, false, IsKnownToBeAllocated, Family)) {
        ...
        // Record the info about the reallocated symbol so that we could
        // properly process failed reallocation.
        stateRealloc =
            stateRealloc->set<ReallocPairs>(ToPtr, ReallocPair(FromPtr, Kind));
        // The reallocated symbol should stay alive for as long as the new
        // symbol.
...
ProgramStateRef MallocChecker::CallocMem(CheckerContext &C,
    ...
    return MallocMemAux(
        C, Call, TotalSize, zeroVal, State, AllocationFamily(AF_Malloc));
}
        ...
        if (!ReferenceRegion) {
            if (const MemRegion *MR = C.getLocationRegionIfPostStore(N)) {
                ...
                if (Val.getAsLocSymbol() == Sym) {
                    const VarRegion *VR =
                        MR->getBaseRegion()->getAs<VarRegion>();
                    // Do not show local variables belonging to a function other
                    // than where the error is reported.
                    if (!VR ||
...
void MallocChecker::HandleLeak(SymbolRef Sym,
                               ExplodedNode *N,
                               CheckerContext &C) const {
    ...
    if (!BT_Leak[*CheckKind]) {
        ...
        //     not to report leaks on such paths.
        BT_Leak[*CheckKind].reset(new BugType(CheckNames[*CheckKind],
                                              "Memory leak",
                                              categories::MemoryError,
                                              /*SuppressOnSink=*/true));
    }
...
void MallocChecker::checkDeadSymbols(SymbolReaper &SymReaper,
    ...
    for (auto [Sym, ReallocPair] : RP) {
        if (SymReaper.isDead(Sym) ||
            SymReaper.isDead(ReallocPair.ReallocatedSym)) {
...
void MallocChecker::checkEscapeOnReturn(const ReturnStmt *S,
    ...
    if (!Sym)
        // If we are returning a field of the allocated struct or an array
        // element, the callee could still free the memory.
...
bool MallocChecker::checkUseAfterFree(SymbolRef Sym,
                                      CheckerContext &C,
                                      const Stmt *S) const {
...
void MallocChecker::checkUseZeroAllocated(SymbolRef Sym,
                                          CheckerContext &C,
                                          const Stmt *S) const {
    assert(Sym);
...
void MallocChecker::checkLocation(SVal l,
                                  bool isLoad,
                                  const Stmt *S,
                                  CheckerContext &C) const {
...
bool MallocChecker::mayFreeAnyEscapedMemoryOrIsModeledExplicitly(
    ...
    if (const ObjCMethodCall *Msg = dyn_cast<ObjCMethodCall>(Call)) {
        ...
        // If there's a "freeWhenDone" parameter, but the method isn't one we
        // know about, we can't be sure that the object will use free() to
        // deallocate the memory, so we can't model it explicitly. The best we
        // can do is use it to decide whether the pointer escapes.
        ...
        // or replacePointer, assume we are dealing with NSPointerArray or
        // similar. This is similar to C++ containers (vector); we still might
        // want to check that the pointers get freed by following the container
        // itself.
        ...
        // We should escape receiver on call to 'init'. This is especially
        // relevant to the receiver, as the corresponding symbol is usually not
    ...
    // even though the address escapes, it's still our responsibility to free
    // the buffer.
...
MallocChecker::checkPointerEscape(ProgramStateRef State,
                                  ...
                                  const CallEvent *Call,
                                  PointerEscapeKind Kind) const {
    return checkPointerEscapeAux(State,
                                 Escaped,
                                 Call,
                                 Kind,
...
ProgramStateRef
MallocChecker::checkPointerEscapeAux(ProgramStateRef State,
                                     const InvalidatedSymbols &Escaped,
                                     const CallEvent *Call,
                                     PointerEscapeKind Kind,
                                     bool IsConstPointerEscape) const {
...
static bool isReferenceCountingPointerDestructor(const CXXDestructorDecl *DD) {
    if (const IdentifierInfo *II = DD->getParent()->getIdentifier()) {
        ...
        if (N.contains_insensitive("ptr") ||
            N.contains_insensitive("pointer")) {
            if (N.contains_insensitive("ref") ||
                N.contains_insensitive("cnt") ||
                N.contains_insensitive("intrusive") ||
                N.contains_insensitive("shared") ||
                N.ends_with_insensitive("rc")) {
                return true;
            }
        }
    }
...
PathDiagnosticPieceRef MallocBugVisitor::VisitNode(const ExplodedNode *N,
    ...
    // original reference count is positive, we should not report
    // use-after-frees on objects deleted in such functions. This can probably
    // be improved through better shared pointer modeling.
        ...
        if (isAllocated(RSCurr, RSPrev, S)) {
            ...
            case AF_InnerBuffer: {
                ...
                if (N->getLocation().getKind() ==
                    ...
                    } else if (const auto *OpCallE =
                                   dyn_cast<CXXOperatorCallExpr>(S)) {
                        OS << OpCallE->getDirectCallee()->getDeclName();
                    } else if (const auto *CallE = dyn_cast<CallExpr>(S)) {
                        auto &CEMgr =
                            BRC.getStateManager().getCallEventManager();
                        CallEventRef<> Call = CEMgr.getSimpleCall(
                            CallE, state, CurrentLC, {nullptr, 0});
                        if (const auto *D =
                                dyn_cast_or_null<NamedDecl>(Call->getDecl()))
                            OS << D->getDeclName();
                        else
                            OS << "unknown";
                    }
            ...
            for (const LocationContext *LC = CurrentLC; LC;
                ...
                if (const auto *DD =
                    ...
                    if (isReferenceCountingPointerDestructor(DD)) {
                        // This immediately looks like a reference-counting
                        // destructor. We're bad at guessing the original
                        // reference count of the object, so suppress the report
                        // for now.
                        BR.markInvalid(getTag(), DD);

                        // After report is considered invalid there is no need
                        // to proceed futher.
                        return nullptr;
                    }

                    // Switch suspection to outer destructor to catch patterns
                    // like: (note that class name is distorted to bypass
                    ...
                    // }
                    //
                    // This way ReleaseFunctionLC will point to outermost
                    // destructor and it would be possible to catch wider range
                    // of FP.
                    //
                    // NOTE: it would be great to support smth like that in C,
                    // since currently patterns like following won't be
                    // supressed:
                    //
            ...
            StackHint =
                std::make_unique<StackHintGeneratorForReallocationFailed>(
                    Sym, "Reallocation failed");

            if (SymbolRef sym = findFailedReallocSymbol(state, statePrev)) {
                // Is it possible to fail two reallocs WITHOUT testing in
                // between?
...
void MallocChecker::printState(raw_ostream &Out,
                               ProgramStateRef State,
                               const char *NL,
